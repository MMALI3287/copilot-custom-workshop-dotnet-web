...
ワークスペースルートに `.github/copilot-instructions.md` を作成します：
...
> **Note:** `/` を入力しても作成したプロンプトが表示されない場合は、`.github/prompts` フォルダが `app/` フォルダの直下にあることを確認してください。このワークショップでは `app/` がワークスペースルートです。`MeowWorld/` など C# プロジェクトのフォルダの下に `.github/prompts` を作成すると、プロンプトが候補に表示されない場合があります。
