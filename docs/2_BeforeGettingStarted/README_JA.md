...
このワークショップでは、Copilot Chat のモデル選択は **Auto** を推奨します。

Auto を選ぶと、Copilot がプロンプト内容や作業内容に応じて適切なモデルを選択します。参加者ごとのライセンスや組織設定によって利用可能なモデルが異なる場合があるため、ハンズオンでは Auto を前提に進めます。

> **Tip:** Agent モードで複数ファイルの実装やテスト修正を行う場面では、必要に応じて利用可能な最新の高性能モデル（例: Claude Sonnet 系、GPT 系など）を選んでも構いません。迷った場合は Auto のままで進めてください。
